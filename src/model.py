...
class SegmentDataset(data.Dataset):
    ...
    def __getitem__(self, item):
        ...
        img = Image.open(path_img).convert('L') # Grayscale
    ...
    class _TwoConvLayers(nn.Module):
        def __init__(self, in_channels, out_channels):
            ...
            self.model = nn.Sequential(
                nn.Conv2d(in_channels, out_channels, 1, padding=1, bias=False),
                nn.ReLU(inplace=True),
                nn.BatchNorm2d(out_channels),
                nn.Conv2d(out_channels, out_channels, 1, padding=1, bias=False),
                nn.ReLU(inplace=True),
                nn.BatchNorm2d(out_channels)
            )
...
d_train = SegmentDataset(r"data", transform_img=tr_img, transform_mask=tr_mask)
...
for _e in range(epochs):
    ...
    for x_train, y_train in train_tqdm:
        ...
        train_tqdm.set_description(f"Epoch [{_e+1}/{epochs}], loss mean={loss_mean:.3f}")

st = model.state_dict()
torch.save(st, 'Model_UNet_Segmentation.tar')
